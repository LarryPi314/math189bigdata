...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	RMSE_list = []
	reg_list = np.random.uniform(0.0, 100, 100)
	W_list = [linreg(X_train, y_train, reg = c) for c in reg_list]
	for index in range(len(reg_list)):
		w_des = W_list[index]
		RMSE_list.append(find_RMSE(w_des, X_val, y_val))
	...
	(reg_opt, bestRMSE) = (float('inf'), float('inf'))
	for ind in range(len(reg_list)):
		if(RMSE_list[ind]<bestRMSE):
			bestRMSE = RMSE_list[ind]
			reg_opt = reg_list[ind]
	return reg_opt
...
def linreg_no_bias(X, y, reg=0.0):
	...
	m = X.shape[0]
	id = np.eye(m)
	agg = X.T @ (id - np.ones(m) / m)
	W_opt = np.linalg.solve(agg @ X + reg * np.eye(agg.shape[0]), \
		agg @ y)
